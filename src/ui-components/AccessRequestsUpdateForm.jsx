...
export default function AccessRequestsUpdateForm(props) {
  const {
    id,
    accessRequests,
    onSuccess,
  ...
  const resetStateValues = () => {
    const cleanValues = { ...initialValues, ...accessRequestsRecord };
    setUsername(cleanValues.username);
    setWebApplicationsRelation(cleanValues.WebApplicationsRelation);
    setRequestdate(cleanValues.requestdate);
    setAccessreason(cleanValues.accessreason);
    setStatus(cleanValues.status);
    setApproverusername(cleanValues.approverusername);
    setApproverreason(cleanValues.approverreason);
    setAccessRequestsWebApplicationsRelationId(
      cleanValues.accessRequestsWebApplicationsRelationId
    );
    setErrors({});
  };
  const [accessRequestsRecord, setAccessRequestsRecord] =
    React.useState(accessRequests);
  React.useEffect(() => {
    const queryData = async () => {
      const record = id
        ? await DataStore.query(AccessRequests, id)
        : accessRequests;
      setAccessRequestsRecord(record);
    };
    queryData();
  }, [id, accessRequests]);
  React.useEffect(resetStateValues, [accessRequestsRecord]);
  const validations = {
    username: [],
    WebApplicationsRelation: [],
    requestdate: [],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          await DataStore.save(
            AccessRequests.copyOf(accessRequestsRecord, (updated) => {
      ...
      {...rest}
      {...getOverrideProps(overrides, "AccessRequestsUpdateForm")}
    >
      <TextField
        label="Username"
        isRequired={false}
      ...
      <TextField
        ...
        isRequired={false}
        isReadOnly={false}
        defaultValue={accessRequestsWebApplicationsRelationId}
        onChange={(e) => {
          let { value } = e.target;
          if (onChange) {
            const modelFields = {
